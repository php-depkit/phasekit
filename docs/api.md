...
- `@depkit/phasekit-opencode/plugin` plugin-only default export for direct imports when needed

The plugin registers native `phasekit_*` tools and relies on `/pk-*` markdown commands and agents generated by an explicit installer run. OpenCode config should reference `@depkit/phasekit-opencode` as the plugin spec.
